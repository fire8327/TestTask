<template>
    <div class="relative group">    
        ...
        <div class="absolute z-50 top-[calc(100%+10px)] shadow-[3px_2px_16px_0px_rgba(0,0,0,0.10)] w-fit flex flex-col rounded-lg transition-all duration-500 bg-white -translate-x-1/2 left-1/2" :class="{'hidden' : !isDown}">
            <NuxtLink @click="closeDropdown" v-for="content in contents" :to="content.link" class="transition-all duration-500 hover:bg-gray-200 px-4 py-1.5 first:rounded-t-lg last:rounded-b-lg">{{ content.title }}</NuxtLink>
        </div>
    </div>
</template>
